...
class HistoryView(context: Context) : LinearLayout(context) {
    ...
    private fun render(runs: JSONArray?) {
        ...
        for (i in 0 until runs.length()) {
            ...
            val row = runRow(r)
            listBox.addView(row)
            UiKit.appear(row, delayMs = (i * 45L).coerceAtMost(400L))
        }
    }
    ...
    private fun runRow(r: JSONObject): LinearLayout {
        ...
        val row = LinearLayout(context).apply {
            ...
            background = with(UiKit) {
                context.tintCard("#FFFFFF", "#E2E5EA")
            }
